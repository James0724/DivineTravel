...
import { SUPPORTED_CURRENCIES, CURRENCY_META, type CurrencyCode } from '@/lib/currency/constants'
  ...
  return (
    <div ref={rootRef} className={cn('relative', className)}>
      ...
      >
        ...
        <span className={variant === 'pill' ? 'font-semibold' : undefined}>{currency}</span>
        <ChevronDown
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={cn(
              'absolute right-0 z-[300] w-[300px] max-h-[min(400px,70vh)] overflow-hidden rounded-xl',
              'border border-bone-ink/12 bg-white shadow-[0_12px_40px_-8px_rgba(0,0,0,0.18)] ring-1 ring-black/5',
            ...
            <div className="max-h-[min(260px,45vh)] overflow-y-auto p-1">
              ...
              {filtered.map((code) => {
                ...
                return (
                  ...
                  >
                    <FlagIcon countryCode={meta.countryCode} className="h-3 w-4" />
                    <span className="w-9 shrink-0 font-bold text-bone-ink/85">{code}</span>
                    <span className="flex-1 truncate text-[10px] text-bone-ink/50">{meta.name}</span>
                    <span className="shrink-0 text-[10px] font-semibold tabular-nums text-bone-ink/40">
                      {meta.symbol}
                    </span>
                    {active && <span className="shrink-0 text-[10px] text-bone-clay">✓</span>}
                  </button>
                )
              })}
            </div>
