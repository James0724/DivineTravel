...
const faqs = [
  ...
  },
  {
    ...
    a: (
      <div>
        <p className="mb-3">A standard Divine Travel Nest Safaris package includes:</p>
        <table className="w-full border-collapse text-[14px]">
          <tbody>
            {[
              ...
              <tr key={item as string} style={{ borderBottom: '1px solid rgba(23,22,18,0.1)' }}>
                <td className="py-2.5 px-3.5">{item}</td>
                <td className="py-2.5 px-3.5 text-right font-serif italic" style={{ color: (inc as string) === '✓' ? '#9d4519' : '#7a7264' }}>{inc}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="container-site">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-10 lg:gap-24 items-start">
          ...
          <div>
            ...
            >
              Safari{' '}
              <em className="italic text-bone-clay">FAQ</em>.
            </h2>
            <p className="text-[14px] text-bone-muted mt-[18px] max-w-[40ch] lg:max-w-[32ch] leading-[1.6]">
              Can't find your answer? Our team is happy to help — call or email us any time.
            </p>
          </div>
                ...
                <AnimatePresence initial={false}>
                  {open === i && (
                    <motion.div
                      ...
                      className="overflow-hidden text-[14px] sm:text-[15px] leading-[1.65] text-bone-muted"
                    >
                      <div className="pb-6">
