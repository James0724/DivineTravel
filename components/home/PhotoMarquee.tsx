...
export default function SafariMarquee() {
  return (
    <section className="relative overflow-hidden">
      <SimpleMarquee baseVelocity={3} repeat={2} slowdownOnHover className="gap-0">
        {photos.map((photo, i) => (
          <article
            key={i}
            className="group relative h-[320px] w-[480px] shrink-0 overflow-hidden border-r border-[var(--line-soft)]"
          >
            <Image
              src={photo.src}
              alt={photo.caption}
            ...
            <div className="absolute bottom-5 left-5 z-[2] font-mono text-[10px] uppercase tracking-[0.18em] text-bone-paper/90">
              <b className="mb-[2px] block font-medium text-[#f4d4a8]">{photo.label}</b>
