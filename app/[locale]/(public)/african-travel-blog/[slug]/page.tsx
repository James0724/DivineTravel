...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: "article",
      url: `/african-travel-blog/${post.slug}`,
...
export default async function AfricanTravelBlogArticlePage({ params }: Props) {
  ...
  return (
    <>
      <BreadcrumbSchema
        items={[
          { name: "Home", href: "/" },
          { name: "African Travel Blog", href: "/african-travel-blog" },
          { name: post.title, href: `/african-travel-blog/${post.slug}` },
        ]}
      />
      <ArticleSchema
        title={post.title}
        description={post.excerpt}
        url={buildAbsoluteUrl(`/african-travel-blog/${post.slug}`)}
      ...
      <section className="article-hero">
        ...
        />
        <div className="inner">
          <Link href="/african-travel-blog" className="back">
            ← Back to the blog
          </Link>
            ...
            >
              {related.map((rp) => (
                <RevealItem key={rp._id} className="flex flex-col gap-4">
                  <Link
                    href={`/african-travel-blog/${rp.slug}`}
